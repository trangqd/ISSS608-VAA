...
pacman::p_load(tidyverse, jsonlite, dplyr, igraph,
               ...
               SmartEDA, widyr, ggforce, tidytext, htmlwidgets,
               webshot2, knitr)
```
...
## Conversion for htmp outputs

VisNetwork produces interactive HTML widgets, which cannot be embedded directly in Word (.docx). Thus, a helper function is created to render the html outputs, take a screenshot of each html output, and insert them into the docx.

```{r}
render_visnetwork_as_image <- function(vis_obj, name = "network", width = 1000, height = 1000) {
  dir.create("images", showWarnings = FALSE)

  html_file <- file.path("images", paste0(name, ".html"))
  img_file  <- file.path("images", paste0(name, ".png"))

  saveWidget(vis_obj, file = html_file, selfcontained = TRUE)
  webshot2::webshot(
    url = html_file,
    file = img_file,
    vwidth = width,
...
network1 <- visNetwork(vis_nodes, vis_edges) %>%
...
img1 <- render_visnetwork_as_image(network1, name = "network1")
...
network2 <- visNetwork(vis_nodes, vis_edges) %>%
...
img2 <- render_visnetwork_as_image(network2, name = "network2")
...
network3 <- visNetwork(vis_nodes, vis_edges) %>%
...
img3 <- render_visnetwork_as_image(network3, name = "network3")
