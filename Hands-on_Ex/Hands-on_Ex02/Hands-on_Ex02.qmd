...
#Beyond ggplot2 Themes ##ggplot2 built-in themes
...
## 1. Boxplot: English Score Distribution by Class

```{r}
p1 <- ggplot(exam_data, aes(x = CLASS, y = ENGLISH)) +
  geom_boxplot(outlier.shape = 16, outlier.size = 1.2, width = 0.7, position = position_dodge(0.8)) +
  scale_fill_brewer(palette = "Set2") +
  labs(title = "Boxplot: English Score by Class", x = "Class", y = "English Score")
p1

```

## 2. Dot Plot: English Score Distribution by Race

```{r}
p2 <- ggplot(exam_data, aes(x = RACE, y = ENGLISH, fill = RACE)) +
  geom_dotplot(
    binaxis = 'y',
    stackdir = 'center',
    dotsize = 0.7,
  ...
  labs(
    title = "Dot Plot: English Score by Race",
    x = "Race",
    y = "English Score"
  ) +
